...
def download_audio(YTID: str, path: str) -> None:
    ...
    ** Utilisez la librairie youtube_dl : https://github.com/ytdl-org/youtube-dl/ **
    ...
    if os.path.exists(path):
        print(f"Le fichier existe déjà à l'emplacement : {path}")
        return
    
    # URL de la vidéo YouTube
    url = f'https://www.youtube.com/watch?v={YTID}'

    # Options pour youtube_dl pour télécharger l'audio en mp3
    ydl_opts = {
        'format': 'bestaudio/best',
        'outtmpl': path,  # Sauvegarde l'audio avec le nom donné dans path
        'postprocessors': [{
            'key': 'FFmpegExtractAudio',
            'preferredcodec': 'mp3',
            'preferredquality': '192',
        }],
        'quiet': True,  # Ne pas afficher les logs détaillés
    }
    
    try:
        # Utiliser youtube_dl pour télécharger l'audio
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ydl.download([url])
        print(f"L'audio a été téléchargé et sauvegardé à {path}")
...
def cut_audio(in_path: str, out_path: str, start: float, end: float) -> None:
    ...
    ** Utilisez la bibliothèque ffmpeg : https://github.com/kkroening/ffmpeg-python
    ...
    # TODO
    try:
    # Utiliser ffmpeg pour couper l'audio du fichier d'entrée
        (
            ffmpeg
            .input(in_path, ss=start, to=end)  # ss=start : début de la coupe, to=end : fin de la coupe
            .output(out_path)  # Spécifier le fichier de sortie
            .run(quiet=True, overwrite_output=True)  # Exécuter et écraser si le fichier existe déjà
        )
        print(f"L'audio a été coupé et sauvegardé dans {out_path}")
    except ffmpeg.Error as e:
        print(f"Une erreur est survenue lors du traitement audio : {e.stderr.decode()}")